...
import { generateOptionsAction, checkAiRunStatusAction, type DerivationState } from "../actions";

const POLL_MS = 2000;

export function GenerateOptionsButton({ engagementId, hasOptions }: { engagementId: string; hasOptions: boolean }) {
  const router = useRouter();
  const [state, action, pending] = useActionState<DerivationState, FormData>(generateOptionsAction, null);
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      ...
      if (res.status === "succeeded") {
        ...
        setPollError(res.error ?? "Generation failed.");
      }
    }, POLL_MS);
