...
import { deriveSwotAction, type FormState } from "../actions";

export function DeriveButton({ engagementId, hasItems }: { engagementId: string; hasItems: boolean }) {
  const [state, action, pending] = useActionState<FormState, FormData>(deriveSwotAction, null);
  return (
    <form action={action} className="flex flex-col items-end gap-1">
      ...
      >
        {pending ? "Deriving with Sonnet 5…" : hasItems ? "Re-derive SWOT" : "Derive SWOT"}
      </button>
