...
- **Handover method: transfer the data, self-deploy the hosting.** The **database** is transferred (Supabase project transfer is free and keeps the same ref/URL/keys/data). The **website is not** — Netlify's *site transfer* is a paid-plan feature, but a fresh deploy is free, so the client deploys it themselves. The chosen path is **Netlify drag-and-drop** (app.netlify.com/drop): while logged in, Netlify detects the Next.js framework and runs the build on drop, so **no terminal, no Node.js, and no GitHub** are required (they download the repo ZIP, unzip, and drag the folder). Because `NEXT_PUBLIC_*` values are baked in at build time, the flow is drop → set the three env vars → drop again. The Anthropic key is set by the client (API billing is per-account and cannot be transferred); net effect on keys is that Supabase keys become the client's by owning the project and the Anthropic key is the one they supply. Code updates are re-published by dragging the updated folder onto the site's Deploys dropzone; connecting a GitHub repo for auto-deploy is an optional upgrade. The Netlify CLI (`netlify deploy --build --prod`) remains a valid alternative, and the from-scratch provisioning (five migrations + optional seed) is retained as a fallback / independent-copy path.
